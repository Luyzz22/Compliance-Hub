...
class AISystemRepository:
    ...
    def _to_domain(row: AISystemTable) -> AISystem:
        return AISystem(
            ...
            has_incident_runbook=row.has_incident_runbook,
            has_supplier_risk_register=row.has_supplier_risk_register,
            has_backup_runbook=row.has_backup_runbook,
            status=row.status,
            intended_purpose=row.intended_purpose,
            training_data_provenance=row.training_data_provenance,
            fria_reference=row.fria_reference,
            provider_name=row.provider_name,
            deployer_name=row.deployer_name,
            provider_responsibilities=row.provider_responsibilities,
            deployer_responsibilities=row.deployer_responsibilities,
            pms_status=row.pms_status or "pending",
            pms_next_review_date=row.pms_next_review_date,
            pms_last_review_date=row.pms_last_review_date,
    ...
    def create(self, tenant_id: str, payload: AISystemCreate) -> AISystem:
        ...
        row = AISystemTable(
            ...
            has_incident_runbook=payload.has_incident_runbook,
            has_supplier_risk_register=payload.has_supplier_risk_register,
            has_backup_runbook=payload.has_backup_runbook,
            status=AISystemStatus.draft,
            intended_purpose=payload.intended_purpose,
            training_data_provenance=payload.training_data_provenance,
            fria_reference=payload.fria_reference,
            provider_name=payload.provider_name,
            deployer_name=payload.deployer_name,
            provider_responsibilities=payload.provider_responsibilities,
            deployer_responsibilities=payload.deployer_responsibilities,
            created_at_utc=now,
            updated_at_utc=now,
        )
