        ...
        if tenant_profile.requires_human_approval:
            actions.append(
                ComplianceAction(
                    ...
                    severity=Severity.medium,
                    rationale="EU-AI-Act-konformer Human-Review wird vor finaler Freigabe erzwungen.",
                )
            )
...
def calculate_tenant_score(action_history: list[ComplianceAction]) -> tuple[int, str, list[str]]:
    ...
    if critical or high:
        recommendations.insert(0, "E-Rechnungs- und Drittland-Fehler mit Lieferanten-SLA beheben.")
